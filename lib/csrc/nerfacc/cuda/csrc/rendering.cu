...
template <typename scalar_t>
__global__ void rendering_alphas_forward_with_transmittance_kernel(
    const uint32_t n_rays,
    const int *packed_info,        // input ray & point indices.
    const scalar_t *alphas,        // input alpha (opacity) values.
    const scalar_t early_stop_eps, // transmittance threshold for early stop
    const scalar_t alpha_thre,     // alpha threshold for emtpy space
    // outputs: should be all-zero initialized
    scalar_t *transmittance, // input transmittance values
    scalar_t *weights)     // the number rendering weights for each sample
{
    CUDA_GET_THREAD_ID(i, n_rays);
    ...
    float alpha_thre)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        alphas.scalar_type(),
        "rendering_alphas_forward_with_transmittance",
        ([&]
         { rendering_alphas_forward_with_transmittance_kernel<scalar_t><<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                n_rays,
                packed_info.data_ptr<int>(),
                alphas.data_ptr<scalar_t>(),
                early_stop_eps,
                alpha_thre,
                transmittance.data_ptr<scalar_t>(),
                weights.data_ptr<scalar_t>()); }));
