...
torch::Tensor unpack_mask(const torch::Tensor packed_info, const int n_samples)
{
    DEVICE_GUARD(packed_info);
    CHECK_INPUT(packed_info);
    ...
    torch::Tensor masks = torch::zeros(
        {n_samples}, packed_info.options().dtype(torch::kBool));

    unpack_mask_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
        n_rays,
        packed_info.data_ptr<int>(),
        masks.data_ptr<bool>());
    return masks;
