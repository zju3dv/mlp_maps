...
import lib.csrc.nerfacc.cuda as _C
...
@torch.no_grad()
def render_weight_from_alpha_with_transmittance(
    packed_info,
    alphas,
    transmittance,
    early_stop_eps: float = 1e-4,
    ...
    if not alphas.is_cuda:
        raise NotImplementedError("Only support cuda inputs.")
    weights = _C.rendering_alphas_forward_with_transmittance(
        packed_info.contiguous(),
        alphas.contiguous(),
        transmittance.contiguous(),
        early_stop_eps,
        alpha_thre,
    )[0]
    return weights
