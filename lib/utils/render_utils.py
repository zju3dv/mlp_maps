...
def gen_path(RT, center=None, render_views=50):
    lower_row = np.array([[0., 0., 0., 1.]])
    ...
    start = cfg.get('render_start', 0)
    length = cfg.get('render_length', 2 * np.pi)
    for theta in np.linspace(start, start + length, render_views + 1)[:-1]:
        # camera position
