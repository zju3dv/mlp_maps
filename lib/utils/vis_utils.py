...
def vis_bbox(img, corners_2d, coord=None):
    _, ax = plt.subplots(1)
    ax.imshow(img)
    ax.add_patch(
...
from lib.utils.if_nerf.if_nerf_data_utils import get_bound_corners
def bbox_img(image, bounds, ext, ixt, linewidth=3, color=(0, 1, 0)):
