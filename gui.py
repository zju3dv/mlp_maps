...
from lib.networks.renderer.dymap_fast_renderer import Renderer as FastRender
from lib.datasets.dymap_interactive import Dataset
...
class Renderer:
    def __init__(self):
        assert cfg.fast_render, 'fast render options is needed'
        ...
        self.dataset = Dataset(**cfg.test_dataset)

        # prepare renderer
        self.renderer = FastRender(self.network)

        self.frame_start = cfg.begin_ith_frame
        self.frame_step = cfg.frame_interval
    ...
    def render_next(self):
        batch = self.dataset[(self.frame_curr, self.cam.get_w2c())]
        if opt.autoplay:
            self.iter += 1
        ...
        ret = self.visualizer.visuliaze_interactive(ret, batch)
...
def glfw_update_title(window):
    ...
    if elapsed > opt.fps_cnter_int:
        ...
        times.append(fps)
        if len(times) == 10:
            title = f"Dynamic MLP Maps: {np.mean(sorted(np.array(times))[2:-2]):.3f} fps"
        else:
            title = f"Dynamic MLP Maps: {fps:.3f} fps"
        # print(title)
